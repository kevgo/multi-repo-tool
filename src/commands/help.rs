...
pub fn help() -> (Config, Option<ExitCode>) {
  ...
  println!("If the given executable fails in one of the subfolders, you end up in that subfolder.");
  ...
  println!("By default, mrt iterates through all direct subfolders of the directory it is called.");
  println!("To limit the set of folders that mrt goes through:");
  println!("m only <condition>   keeps only the folders for which condition returns exit code 0.");
  println!(
